...
3. 相手チームのメンバーと相対し、双方 2 名生きていれば、協議の上 1 vs 1 を
   決定する。そうでない場合、自動的に 1 vs 1 もしくは 1 vs 2 が成立する。
...
2. 入手したライフトークンは以下の用途で消費する。
   - 脱落メンバーのサルベージ: 入手したライフトークンを復活したメンバーにそのまま割り当てる。つまりライフ 1 の状態で再スタート。また、ボーナス
     として山から新しい属性カードを 1 枚得られる。
