...
- Does NOT contain any
  `<!-- oneiron-roadmap-id: … -->` marker (the issue
  is not itself a roadmap).
- Does NOT contain any
  `<!-- oneiron-blocked-by: … -->` marker.
...
**A0-T** (the scoped `oneiron-roadmap-id` lookup
needed to resolve the explicit target's
`oneiron-blocked-by` markers),
...
for `oneiron-roadmap-id` to resolve
`oneiron-blocked-by` dependency markers; see A2 for
...
or containing an
`<!-- oneiron-roadmap-id: ... -->` marker is a
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `oneiron-blocked-by` markers on the explicit target.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `oneiron-roadmap-id` marker when checking
  `oneiron-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Check both
  ...
  `<!-- oneiron-blocked-by: {roadmap-id} -->` markers
  — for each `{roadmap-id}`, find the issue whose body contains
  `<!-- oneiron-roadmap-id: {roadmap-id} -->`. If that
...
4. **A3 filtered to zero** (A2 found execution candidates but all were
   ...
   **Diagnostic — all candidates blocked by an open roadmap**: if every
   ...
   `<!-- oneiron-blocked-by: X -->` marker that points
...
- **Roadmap identity** (`oneiron-roadmap-id`): placed
...
- **Sequential dependency** (`oneiron-blocked-by`):
...
**Do not use `oneiron-blocked-by` to group sub-tasks
